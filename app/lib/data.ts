...
const sql = createKysely<Database>();
...
export async function fetchRevenue() {
  try {
    ...
    const data = await sql.selectFrom('revenue').selectAll().execute();
    console.log('Done fetching revenue data.');
    return data;
  } catch (error) {
...
export async function fetchLatestInvoices() {
  try {
    const data = await sql.selectFrom('invoices')
...
export async function fetchCardData() {
  try {
    ...
    //      FROM invoices`;
     ...
     const invoiceCountPromise = await sql
                                    .selectFrom('invoices')
                                    .select((eb) => eb.fn.countAll().as('count'))
                                    .executeTakeFirst();

    const customerCountPromise = await sql
